...
public class EasyPermission {
    ...
    private static List<ModelPermission> mPermissionListNeedReq;//未被授权的权限列表
    ...
    public static void handleResult(Activity obj, int requestCode, String[] permissions, int[] grantResults) {
        ...
        List<ModelPermission> needRationalPermissionList = new ArrayList<ModelPermission>();
        //所有被禁用的权限
        List<ModelPermission> deniedPermissionList = new ArrayList<ModelPermission>();
        ...
        if (needRationalPermissionList != null && needRationalPermissionList.size() > 0) {
            ...
            for (ModelPermission info : needRationalPermissionList) {
