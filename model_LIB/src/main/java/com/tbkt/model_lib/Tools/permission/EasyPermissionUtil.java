...
public class EasyPermissionUtil {
     ...
     */
    @TargetApi(value = Build.VERSION_CODES.M)
    public static List<ModelPermission> findRationalePermissions(Activity activity, String... permission) {
        List<ModelPermission> mPermissionListNeedReq = new ArrayList<ModelPermission>();

        for (String value : permission) {
            if (activity.checkSelfPermission(value) != PackageManager.PERMISSION_GRANTED) {
                //判断是否勾选“不再提示”
                ModelPermission info = new ModelPermission();
                info.setName(value);
